...
require.config({
    ...
    paths: {
        ...
        'upload': [tapiRoot + '/api/upload/index.js?'],
...
$(function () {
    ...
    $.msg = new function () {
        ...
        this.loading = function (msg, call) {
            ...
            }) : layer.load(0, {time: 0, scrollbar: false, shade: this.shade, end: call});
            return $.msg.idx.push(idx), idx;
        };
        /*! Notify 调用入口 */
        ...
        this.notify = function (title, message, time, option) {
            require(['notify'], function (Notify) {
                Notify.notify(Object.assign({
                    title: title || '',
                    description: message || '',
                    position: 'top-right',
                    closeTimeout: time || 3000
                }, (option || {})));
            });
        };
        /*! 页面加载层 */
    ...
    $.form = new function () {
        ...
        this.load = function (url, data, method, callable, loading, tips, time, headers) {
            ...
            if (typeof data === 'object' && typeof data['_token_'] === 'string') {
                headers['User-Form-Token'] = data['_token_'], delete data['_token_'];
            }
            ...
            $.ajax({
                data: data || {}, type: method || 'GET', url: $.menu.parseUri(url), beforeSend: function (xhr, i) {
                    ...
                    // 异常消息显示处理
                    if (parseInt(XMLHttpRequest.status) !== 200 && XMLHttpRequest.responseText.indexOf('exception') > -1) try {
                        layIdx = layer.open({
                    ...
                    if (typeof callable === 'function' && callable.call($.form, XMLHttpRequest, time) === false) return false;
                    let httpStatusCode = parseInt(XMLHttpRequest.status)
                    if (httpStatusCode !== 200) {
                        // 处理 422 消息
                        switch (httpStatusCode) {
                            case 422:
                                $.msg.notify('Invalid data', XMLHttpRequest.responseJSON.message || 'Invalid data', 3000, {
                                    position: 'top-center',
                                    type: 'error'
                                })
                                break;
                            case 429:
                                $.msg.tips('E' + XMLHttpRequest.status + ' - Too Many Attempts.');
                                break;
                            case 401:
                                // 登录失效
                                $.msg.error('You are Unauthorized', 3, () => {
                                    location.reload();
                                });
                                break;
                            default:
    ...
    $.menu = new function () {
        ...
        this.sync = function (mode) {
            $('[data-submenu-layout]').map(function () {
                ...
                if (mode === 1) layui.data('AdminMenuState', {
                    key: node,
    ...
    $.fn.uploadOneVideo = function () {
        return this.each(function () {
            ...
            $bt.on('click', 'i.layui-icon-search', function (event) {
                ...
                'data-size': $in.data('size') || 0,
                'data-type': $in.data('type') || 'mp4',
            });
        });
    };
    ...
    $.fn.uploadOneImage = function () {
        return this.each(function () {
            ...
            $bt.on('click', 'i.layui-icon-search', function (event) {
                ...
                'data-path': $in.data('path') || '',
                'data-size': $in.data('size') || 0,
                'data-type': $in.data('type') || 'gif,png,jpg,jpeg',
                'data-max-width': $in.data('max-width') || 0,
                'data-max-height': $in.data('max-height') || 0,
                'data-cut-width': $in.data('cut-width') || 0,
                'data-cut-height': $in.data('cut-height') || 0,
            });
        });
    };
        ...
        this.render = function (tabldId) {
            ...
            function bindData(options) {
                ...
                if (sort.field && sort.type) {
                    ...
                    if (sform) $(sform).find('[data-form-export]').attr({
    ...
    $.previewPhonePage = function (href, title) {
        ...
        layer.style(layer.open({
            ...
            content: laytpl(template).render({title: title || '公众号', url: href})
        }), {boxShadow: 'none'});
    };
    ...
    onEvent('click', '[data-load]', function () {
        applyRuleValue(this, {}, function (data, elem, dset) {
            $.form.load(dset.load, data, dset.method || 'get', onConfirm.getLoadCallable(dset.tableId), true, dset.tips, dset.time);
        });
    });
    ...
    onEvent('click', '[data-tips-image]', function (event) {
        (event.items = [], event.$imgs = $(this).parent().find('[data-tips-image]')).map(function () {
            ...
            anim: 5,
            closeBtn: 1,
            photos: {start: event.$imgs.index(this), data: event.items},
            tab: function (pic, $ele) {
