    ...
    <ul class="layui-nav layui-layout-right">
        <li class="layui-nav-item">
            <a>
                ...
                <cite>这里一条最新的通知，请查看！</cite>
            </a>
        </li>
        <li lay-unselect class="layui-nav-item"><a data-reload><i class="layui-icon layui-icon-refresh-3"></i></a>
        </li>
        <li lay-unselect class="layui-nav-item layout-top-switch"
            data-value="status#{{ auth()->user()->status }}"
            data-action="{{ route('as.user.status') }}"
        >
            <input type="checkbox" name="work_online" {{ auth()->user()->status == 1 ? 'checked' : '' }}
            lay-skin="switch" lay-text="Working|Resting">
        </li>
