...
allowMime = JSON.parse('@json($exts)');
...
this.$inputIds = $('input[name="' + this.option.elem.data('field') + '_ids"]:not([type=file])');
this.option.elem.data('input_ids', this.$inputIds.size() > 0 ? this.$inputIds.get(0) : null);
}
if (!this.option.elem.data('input_ids') && this.option.elem.data('field')) {
this.$inputIds = $('input[name="' + this.option.elem.data('field') + '_ids"]:not([type=file])');
this.option.elem.data('input_ids', this.$inputIds.size() > 0 ? this.$inputIds.get(0) : null);
}
...
$(this.option.elem.data('input_ids')).val(file.id).trigger('change', file);
}
// 文件上传成功事件
...
var ids = this.option.elem.data('input_ids').value || [];
if (typeof urls === 'string') urls = urls.split('|');
for (var i in this.files) {urls.push(this.files[i].xurl);ids.push(this.files[i].id)};
$(this.option.elem.data('input')).val(urls.join('|')).trigger('change', this.files);
$(this.option.elem.data('input_ids')).val(ids.join('|')).trigger('change', this.files);
}
...
var that = this;
this.notify = Notify.notify({width: 260, title: file.name, showProgress: true, description: '上传进度 <span
    data-upload-progress>0%</span>', type: 'default', position: 'top-right', closeTimeout: 0});
